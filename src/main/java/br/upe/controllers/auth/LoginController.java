            ...
            try {
                ...
                if (user != null) {
                    if (user.getPassword().equalsIgnoreCase(senha)) {
                        ...
                        DatabaseContext dbContextUR = DatabaseUtils.getDatabaseContext();
                        UserRole userRole = dbContextUR.getUserRoles().Find(user.getId());

                        System.out.println("User from DB: "+user.toString());
                        System.out.println("UserRole from DB: "+userRole.toString());

                        if(userRole.getName().equalsIgnoreCase("doctor")){
                            response.sendRedirect("/views/logged/dashboard-doctor.jsp");
